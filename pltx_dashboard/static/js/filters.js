...
function syncFilterVisibility() {
    ...
    var asinGroup = document.getElementById('asinSelect')
        ...
        : null;
    var parentAsinGroup = document.getElementById('parentAsinSelect')
        ? document.getElementById('parentAsinSelect').closest('.topbar-filter-group')
        : null;
    var fsnGroup = document.getElementById('fsnFilterGroup');

    if (platform === 'Flipkart') {
        // Only Flipkart: hide ASIN and Parent ASIN, show FSN
        if (asinGroup) asinGroup.style.display = 'none';
        if (parentAsinGroup) parentAsinGroup.style.display = 'none';
        if (fsnGroup)  fsnGroup.style.display  = '';
    } else if (platform === 'Amazon') {
        // Only Amazon: show ASIN and Parent ASIN, hide FSN
        if (asinGroup) asinGroup.style.display = '';
        if (parentAsinGroup) parentAsinGroup.style.display = '';
