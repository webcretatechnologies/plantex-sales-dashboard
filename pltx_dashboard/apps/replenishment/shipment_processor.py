...
def generate_shipment_report(shipment_file, mapping_file, output_file):
    ...
    df["Appointment Pending Qty"] = df.apply(
        lambda x: x["FINAL QTY"] if x["STATUS_CLEAN"] == "Appointment Pending" else 0,
        axis=1,
    )
    ...
    df["APPOINTMENT DATE"] = (
        df["APPOINTMENT DATE"].astype(str).replace("NaT", "None").replace("", "")
    )
    ...
    df["Shipment_Detail"] = df.apply(
        lambda x: format_appt(x["ID"], x["APPOINTMENT DATE"]), axis=1
    )
    ...
    df["DFC Next Arrival Date"] = df.apply(
        lambda x: (
            x["APPT_DATE_DT"]
            if (
                x["FC Replenishment Type"] == "DFC"
                and pd.notna(x["APPT_DATE_DT"])
                and x["APPT_DATE_DT"] >= today
            )
            else pd.NaT
        ),
        axis=1,
