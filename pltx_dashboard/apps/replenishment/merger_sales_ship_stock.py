...
def generate_master_report(sales_file, stock_file, shipment_file, product_details_file, business_report_file, database_file, raw_sales_file, pin_code_file, fc_mapping_file, input_sheet_file, output_file):
    ...
    try:
        raw_sales_df = pd.read_csv(raw_sales_file, dtype={'Shipment To Postal Code': str})
        pin_code_df = pd.read_csv(pin_code_file, dtype={'PIN CODE': str})
        fc_mapping_df = load_data(fc_mapping_file)
        print(f" Loaded Raw Sales ({len(raw_sales_df)} rows), Pin Codes ({len(pin_code_df)} rows), and FC Mapping ({len(fc_mapping_df)} rows)")

        # Clean postal codes
        raw_sales_df['Shipment To Postal Code'] = raw_sales_df['Shipment To Postal Code'].str.replace(r'\.0$', '', regex=True).str.strip()
        pin_code_df['PIN CODE'] = pin_code_df['PIN CODE'].str.replace(r'\.0$', '', regex=True).str.strip()
        
        # Merge to get Ideal Cluster
        raw_sales_merged = pd.merge(raw_sales_df, pin_code_df[['PIN CODE', 'Fulfilment Cluster', 'Ideal Cluster']], left_on='Shipment To Postal Code', right_on='PIN CODE', how='left')
        ...
        ideal_mapping = raw_sales_merged.groupby(['ASIN', 'Fulfilment Cluster'])['Ideal Cluster'].first().reset_index()
        ideal_mapping.rename(columns={'Fulfilment Cluster': 'Ideal Cluster', 'Ideal Cluster': 'Ideal Cluster name'}, inplace=True)
        ideal_mapping['Ideal Cluster'] = ideal_mapping['Ideal Cluster'].astype(str)
    except Exception as e:
        print(f" Error generating Ideal Cluster mapping: {e}")
        ideal_mapping = pd.DataFrame(columns=['ASIN', 'Cluster Name', 'Ideal Cluster name'])
    ...
    try:
        ...
        def get_input_val(particular, default):
            try:
                val = input_sheet_df[input_sheet_df["Particular"] == particular]["Value"].values[0]
                return val
            except:
                return default
        ...
        stock_date_val = get_input_val("Stock Report Date", pd.Timestamp.now().strftime('%Y-%m-%d'))
        stock_report_date = pd.to_datetime(stock_date_val)
        print(f"  Parameters: P0={p0_days}, P1={p1_days}, P2={p2_days}, Stock Date={stock_report_date.date()}")
    except Exception as e:
    ...
    for df, cluster_col in [(sales_df, 'Cluster Name'), (stock_df, 'Cluster Name'), (ship_df, 'Cluster')]:
        if not df.empty:
    ...
    print(f" Found {len(asin_list)} unique ASINs and {len(cluster_list)} unique Clusters.")
    ...
    master_df = pd.merge(master_df, ideal_mapping, on=['ASIN', 'Ideal Cluster'], how='left')
    ...
    if not sales_df.empty:
        ...
        master_df = pd.merge(master_df, sales_subset, on=['ASIN', 'Ideal Cluster'], how='left')
    else:
        master_df['DRR'] = 0
    ...
    if not stock_df.empty:
        ...
        stock_agg = stock_df.groupby(['ASIN', 'Ideal Cluster'], as_index=False)[stock_cols].sum()
        stock_agg.rename(columns={'Ending Warehouse Balance': 'Stock Qty'}, inplace=True)
        if 'In Transit Between Warehouses' in stock_agg.columns:
            stock_agg.rename(columns={'In Transit Between Warehouses': 'Stock Transfer Qty'}, inplace=True)
        master_df = pd.merge(master_df, stock_agg, on=['ASIN', 'Ideal Cluster'], how='left')
    else:
    ...
    if not ship_df.empty:
        ...
        # Merge DFC
        dfc_cols = ['ASIN', 'Ideal Cluster', 'DFC Appointment Pending Qty', 'DFC Upcoming Shipment Qty', 'DFC Receiving + Intransit Qty', 'DFC Receiving Qty', 'DFC Intransit Qty', 'DFC Shipment ID', 'DFC Next Arrival Date']
        dfc_merge_subset = dfc_df[[c for c in dfc_cols if c in dfc_df.columns]]
        master_df = pd.merge(master_df, dfc_merge_subset, on=['ASIN', 'Ideal Cluster'], how='left')

        # Rename IXD columns
        ixd_df.rename(columns={
        ...
        # Merge IXD
        ixd_cols = ['ASIN', 'Ideal Cluster', 'IXD Appointment Pending Qty', 'IXD Upcoming Shipment Qty', 'IXD Receiving + Intransit Qty', 'IXD Receiving Qty', 'IXD Intransit Qty', 'IXD Shipment ID']
        ixd_merge_subset = ixd_df[[c for c in ixd_cols if c in ixd_df.columns]]
        master_df = pd.merge(master_df, ixd_merge_subset, on=['ASIN', 'Ideal Cluster'], how='left')
    else:
        # Create missing columns
        shipment_empty_cols = [
            'DFC Appointment Pending Qty', 'DFC Upcoming Shipment Qty', 'DFC Receiving + Intransit Qty', 'DFC Receiving Qty', 'DFC Intransit Qty', 'DFC Shipment ID', 'DFC Next Arrival Date',
            'IXD Appointment Pending Qty', 'IXD Upcoming Shipment Qty', 'IXD Receiving + Intransit Qty', 'IXD Receiving Qty', 'IXD Intransit Qty', 'IXD Shipment ID'
        ]
    ...
    if not product_details_df.empty:
        ...
        prod_cols = ['ASIN', 'SKU', 'HSN CODE', 'VENDOR NAME', 'PRODUCTS STATUS', 'ACT WEIGHT', 'VOLUMETRIC WEIGHT', 'PRODUCT TYPE', 'PRODUCT SIZE', 'Portfolio', 'Category']
        prod_subset = product_details_df[[c for c in prod_cols if c in product_details_df.columns]].drop_duplicates(subset=['ASIN'])
        master_df = pd.merge(master_df, prod_subset, on='ASIN', how='left')
    else:
        for col in ['SKU', 'HSN CODE', 'VENDOR NAME', 'PRODUCTS STATUS', 'ACT WEIGHT', 'VOLUMETRIC WEIGHT', 'PRODUCT TYPE', 'PRODUCT SIZE', 'Portfolio', 'Category']:
            master_df[col] = ''
    ...
    if not business_df.empty:
        ...
        business_cols = ['(Child) ASIN', 'Page Views - Total', 'Units Ordered', 'Ordered Product Sales', 'Total Order Items']
        b_subset = business_df[[c for c in business_cols if c in business_df.columns]].drop_duplicates(subset=['(Child) ASIN']).copy()
        ...
        for col in ['(Child) ASIN', 'Page Views - Total', 'Units Ordered', 'Ordered Product Sales', 'Total Order Items']:
            master_df[col] = ''
    ...
    if not database_df.empty:
        ...
        master_df['LIS_CLUSTER'] = master_df['Ideal Cluster'].map(lis_mapping).fillna('')
        
        db_cols = ['ASIN', 'Cluster', 'Sum of Local Shipped Units', 'Sum of Total Units']
        db_subset = database_df[[c for c in db_cols if c in database_df.columns]].copy()
        
        db_subset.rename(columns={
            'Cluster': 'LIS_CLUSTER',
            'Sum of Local Shipped Units': 'LIS_LOCAL_QTY',
            'Sum of Total Units': 'LIS_TOTAL_QTY'
        }, inplace=True)
        
        db_agg = db_subset.groupby(['ASIN', 'LIS_CLUSTER'], as_index=False)[['LIS_LOCAL_QTY', 'LIS_TOTAL_QTY']].sum()
    ...
    numeric_columns = [
        'DRR', 'Sales Qty', 'Stock Qty', 'Stock Transfer Qty', 'Receiving Qty',
        'DFC Appointment Pending Qty', 'DFC Upcoming Shipment Qty', 'DFC Receiving + Intransit Qty', 'DFC Receiving Qty', 'DFC Intransit Qty',
        'IXD Appointment Pending Qty', 'IXD Upcoming Shipment Qty', 'IXD Receiving + Intransit Qty', 'IXD Receiving Qty', 'IXD Intransit Qty',
        'LIS_LOCAL_QTY', 'LIS_TOTAL_QTY',
        'Page Views - Total', 'Units Ordered', 'Ordered Product Sales', 'Total Order Items'
    ]
    ...
    master_df['Receiving Qty'] = master_df.get('DFC Receiving Qty', 0) + master_df.get('IXD Receiving Qty', 0)
    
    # Stock Transfer Qty comes strictly from Stock Report ('In Transit Between Warehouses')
    master_df['Stock Transfer Qty'] = master_df.get('Stock Transfer Qty', 0) # if it already existed from stock_df merge
    ...
    # --- Calculate P0, P1, P2 ---
    master_df['P0'] = (master_df['DRR'] * p0_days - master_df['Stock Qty'] - master_df['Receiving Qty'] - master_df['Stock Transfer Qty'] - master_df['Shipment Qty']).clip(lower=0).round(2)
    master_df['P1'] = (master_df['DRR'] * p1_days - master_df['Stock Qty'] - master_df['Receiving Qty'] - master_df['Stock Transfer Qty'] - master_df['Shipment Qty'] - master_df['P0']).clip(lower=0).round(2)
    master_df['P2'] = (master_df['DRR'] * p2_days - master_df['Stock Qty'] - master_df['Receiving Qty'] - master_df['Stock Transfer Qty'] - master_df['Shipment Qty'] - master_df['P1'] - master_df['P0']).clip(lower=0).round(2)
    ...
    days_since_stock = (today - stock_report_date.normalize()).days
    
    # Days of Cover
    master_df['Days of Cover'] = (master_df['Stock Qty'] / master_df['DRR']).replace([np.inf, -np.inf], 999).fillna(0)
    
    # Stock Out Date Calculation
    days_stock_recv = ((master_df['Stock Qty'] + master_df['Receiving Qty']) / master_df['DRR']).replace([np.inf, -np.inf], 999).fillna(999)
    days_all = ((master_df['Stock Qty'] + master_df['Receiving Qty'] + master_df['Shipment Qty']) / master_df['DRR']).replace([np.inf, -np.inf], 999).fillna(999)
    
    expected_out_date_dt = today + pd.to_timedelta(days_stock_recv, unit='D')
    shipment_date_dt = pd.to_datetime(master_df.get('DFC Next Arrival Date', ''), errors='coerce')
    ...
    def get_stock_out_date(days):
        if pd.isna(days) or days >= 999: return "Never"
...
if __name__ == "__main__":
    ...
    BUSINESS_REPORT_FILE = os.path.join(CURRENT_DIR, "Files", "BusinessReport-02-04-26.csv")
    ...
    generate_master_report(SALES_FILE, STOCK_FILE, SHIPMENT_FILE, PRODUCT_DETAILS_FILE, BUSINESS_REPORT_FILE, DATABASE_FILE, RAW_SALES_FILE, PIN_CODE_FILE, FC_MAPPING_FILE, INPUT_SHEET_FILE, OUTPUT_FILE)
