...
from apps.dashboard.services.analytics_services_orm import generate_kpis_orm, generate_charts_data_orm
from apps.dashboard.services.analytics_services_orm_tables import generate_bi_data_orm, generate_grouped_table_orm
...
def apply_global_filters_orm(qs, filters):
    ...
    if date_range and date_range != 'custom':
        ...
        if date_range == 'yesterday':
            ...
            start = today - datetime.timedelta(days=6); end = today
        elif date_range == 'last_15_days':
            start = today - datetime.timedelta(days=14); end = today
...
def get_available_filters_orm(qs, fk_qs):
    ...
    def clean_qs_vals(qs, field):
        return sorted(set(
            str(v) for v in qs.values_list(field, flat=True).distinct()
...
def run_orm_computation(qs, fk_qs, spend_qs, filters, user, cached_filter_metadata=None):
    ...
    cat_perf_list = [
        {
            ...
            'contribution': round(r['revenue'] / total_rev * 100, 1) if total_rev > 0 else 0
        }
        for r in cat_perf
    ]
    ...
    if tacos and tacos > 15:
        priorities.append({'rank': rank, 'title': 'Reduce Ad Spend', 'subtitle': f'TACoS is high at {tacos:.1f}%. Review and pause non-performing campaigns.', 'priority': 'High'})
        rank += 1

    if oos_count > 0:
        priorities.append({'rank': rank, 'title': f'Restock {oos_count} Out-of-Stock SKUs', 'subtitle': f'You have {oos_count} SKUs with zero units. Act now to prevent lost sales.', 'priority': 'High'})
        rank += 1

    if low_stock_count > 0:
        priorities.append({'rank': rank, 'title': f'Replenish {low_stock_count} Low-Stock SKUs', 'subtitle': 'SKUs with low inventory may run out soon. Trigger replenishment orders.', 'priority': 'Medium'})
        rank += 1

    revenue_change = kpis.get('revenue_change', 0)
    if revenue_change < -5:
        priorities.append({'rank': rank, 'title': 'Investigate Revenue Drop', 'subtitle': f'Revenue declined {abs(revenue_change):.1f}% vs previous period. Review product and ad performance.', 'priority': 'High'})
        rank += 1
    elif revenue_change > 15:
        priorities.append({'rank': rank, 'title': 'Capitalize on Revenue Growth', 'subtitle': f'Revenue up {revenue_change:.1f}%. Consider increasing inventory and ads for top SKUs.', 'priority': 'Medium'})
        rank += 1

    gross_margin_pct = kpis.get('gross_margin_pct', 0)
    if gross_margin_pct < 20:
        priorities.append({'rank': rank, 'title': 'Improve Gross Margins', 'subtitle': f'Gross margin is at {gross_margin_pct:.1f}%. Review pricing, COGS, and promotions.', 'priority': 'Medium'})
        rank += 1

    if not priorities:
        priorities.append({'rank': 1, 'title': 'Review Dashboard Metrics', 'subtitle': 'All key indicators are within normal range. Monitor daily.', 'priority': 'Low'})
