...
def generate_bi_data_orm(qs, fk_qs):
    ...
    if qs is not None:
        agg = qs.values('asin').annotate(
            revenue=Sum('revenue'), total_spend=Sum('total_spend'), orders=Sum('orders'),
            pageviews=Sum('pageviews'), units=Sum('units'),
            spend_sp=Sum('spend_sp'), spend_sb=Sum('spend_sb'), spend_sd=Sum('spend_sd')
        )
        for r in agg:
            az_asins[r['asin']] = {'asin': r['asin'], 'revenue': float(r['revenue'] or 0), 'total_spend': float(r['total_spend'] or 0), 'orders': int(r['orders'] or 0), 'pageviews': int(r['pageviews'] or 0), 'units': int(r['units'] or 0), 'spend_sp': float(r['spend_sp'] or 0), 'spend_sb': float(r['spend_sb'] or 0), 'spend_sd': float(r['spend_sd'] or 0)}

    if fk_qs is not None:
        agg_fk = fk_qs.values('fsn').annotate(
            revenue=Sum('taxable_value'), total_spend=Sum('total_spend'), orders=Sum('orders'),
            pageviews=Sum('pageviews'), units=Sum('units')
        )
        for r in agg_fk:
            fsn = r['fsn']
            if fsn in az_asins:
                ...
                az_asins[fsn] = {'asin': fsn, 'revenue': float(r['revenue'] or 0), 'total_spend': float(r['total_spend'] or 0), 'orders': int(r['orders'] or 0), 'pageviews': int(r['pageviews'] or 0), 'units': int(r['units'] or 0), 'spend_sp': 0.0, 'spend_sb': 0.0, 'spend_sd': 0.0}
...
def generate_period_table_orm(qs, fk_qs, group_col):
    ...
    if qs is not None:
        ...
        for r in agg:
            ...
            if dt not in group_data[key]: group_data[key][dt] = {'revenue':0, 'orders':0, 'units':0, 'pageviews':0, 'spends':0}
...
def generate_grouped_table_orm(qs, fk_qs, group_col):
    ...
    if qs is not None:
        ...
        for r in agg:
            ...
            if k not in group_data: group_data[k] = {'name':k, 'revenue':0, 'orders':0, 'units':0, 'pageviews':0, 'spends':0}
