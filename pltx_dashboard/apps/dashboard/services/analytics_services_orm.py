from django.db.models import Sum, Count, F, Q, Avg, FloatField
...
def generate_kpis_orm(qs, fk_qs, spend_qs=None):
    ...
    return {
        ...
        # Rates
        'roas': round(roas, 2), 'conversion': round(conversion, 2),
        'aov': round(aov, 2), 'tacos': round(tacos, 2),
        # Margins
        'cogs': cogs,
        ...
        'mom_growth': 0, 'yoy_growth': 0,
        'prev_mom': 0, 'prev_yoy': 0,
        'revenue_change': 0, 'orders_change': 0, 'units_change': 0,
        'spend_change': 0, 'aov_change': 0, 'tacos_change': 0,
        'profit_change': 0, 'gross_margin_change': 0,
        'contribution_margin_change': 0,
        'orders_pct_change': 0, 'units_pct_change': 0,
        'run_rate_pct': 0, 'run_rate_status': 'on_track',
    }
...
def generate_charts_data_orm(qs, fk_qs):
    ...
    if qs is not None:
        qs_trend = qs.values('date').annotate(
            revenue=Sum('revenue'),
    ...
    if fk_qs is not None:
        fk_trend = fk_qs.values('date').annotate(
        ...
        for r in fk_trend:
            ...
            if dt not in merged_trend:
                merged_trend[dt] = {'revenue': 0.0, 'total_spend': 0.0, 'pageviews': 0, 'orders': 0}
...
def get_dashboard_payload_orm(qs, fk_qs, spend_qs, filters, user, cached_filter_metadata=None):
    """
